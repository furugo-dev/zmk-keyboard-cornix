...
| 項目       | 仕様                            |
| ---------- | ------------------------------- |
| MCU        | nRF52840（Ebyte E73-2G4M08S1C） |
| キー数     | 50キー（左右各25キー）          |
| スイッチ   | Kailh Choc v2                   |
...
| 段       | 左1 | 左2   | 左3       | 左4       | 左5       | 左6 | ─   | 右1 | 右2       | 右3       | 右4   | 右5   | 右6 |
| -------- | --- | ----- | --------- | --------- | --------- | --- | --- | --- | --------- | --------- | ----- | ----- | --- |
| **上段** | -   | `Esc` | `L`       | `U`       | `F`       | `.` | ─   | `;` | `W`       | `R`       | `Y`   | -     | -   |
| **中段** | `Q` | `E`/⌃ | `I`/⌥     | `A`/⌘     | `O`       | -   | ─   | `K` | `T`       | `N`/⌘     | `S`/⌥ | `H`/⌃ | `P` |
| **下段** | -   | `Z`   | `X`       | `C`       | `V`       | `,` | ─   | `G` | `D`       | `M`       | `J`   | `B`   | -   |
| **親指** | -   | -     | `Esc`/ADJ | `Tab`/LOW | `⇧`/`Spc` |     | ─   |     | `⇧`/`Ent` | `BSP`/RAI | -     | -     |     |

> 表記: `キー`/修飾子 = タップでキー、ホールドで修飾キー。`キー`/LAYER = タップでキー、ホールドでレイヤー切替。
...
| -------- | --- | --- | --- | ----------- | -------------- | --- | --- | --- | -------------- | ----------- | --- | --- | --- |
| **上段** | -   | き  | て  | し          | ←              | -   | ─   | →   | さ             | る          | す  | -   | -   |
| **中段** | †   | ろ  | け  | と          | か             | っ  | ─   | く  | あ             | い          | う  | ー  | へ  |
| **下段** | -   | ほ  | ひ  | は          | こ             | そ  | ─   | た  | な             | ん          | ら  | れ  | -   |
| **親指** | -   | -   | -   | `Tab`/LOWER | **⇧**(`Space`) |     | ─   |     | **⇧**(`Enter`) | `BSP`/RAISE |     |     |     |

#### シフトあり（Space または Enter 同時押し）

| 段       | 左1 | 左2 | 左3 | 左4 | 左5 | 左6 | ─   | 右1 | 右2  | 右3 | 右4 | 右5 | 右6 |
| -------- | --- | --- | --- | --- | --- | --- | --- | --- | ---- | --- | --- | --- | --- |
| **上段** | -   | め  | り  | ね  | ⇧←  | -   | ─   | ⇧→  | じゅ | よ  | え  | ふ  | ゆ  |
| **中段** | -   | せ  | み  | に  | ま  | ち  | ─   | や  | の   | も  | つ  | -   | -   |
| **下段** | -   | ほ  | ひ  | を  | 、↩ | ぬ  | ─   | お  | 。↩  | む  | わ  | れ  | -   |

> † は単独では出力なし。他のキーと組み合わせて小書きかな（ゃゅょぁぃぅぇぉ）を入力するプレフィックス。
>
> `Space`/`Enter` は薙刀式内で同じシフトビットに割り当てられており、左右の親指どちらでもシフトとして機能する。
>
> 薙刀式の文字配列の詳細は zmk-naginata-v17 ライブラリを参照。
...
| 段       | 左1 | 左2 | 左3 | 左4                                                                                                    | 左5 | 左6 | ─   | 右1  | 右2  | 右3  | 右4   | 右5   | 右6   |
| -------- | --- | --- | --- | ------------------------------------------------------------------------------------------------------ | --- | --- | --- | ---- | ---- | ---- | ----- | ----- | ----- |
| **上段** | -   | -   | `[` | `"`                                                                                                    | `]` | -   | ─   | `F5` | `F4` | `F2` | `F10` | `F11` | `F12` |
| **中段** | `@` | `^` | `(` | `'`                                                                                                    | `)` | `$` | ─   | `←`  | `↓`  | `↑`  | `→`   | `Del` | -     |
| **下段** | -   | `&` | `{` | `` ` `` | `}` | `_` | ─   | `Home` | `PgUp` | `PgDn` | `End` | -     | -     |  |  |  |  |  |  |  |  | |     |     |     |      |      |      |       |       |       |
| **親指** | -   | -   | ←   | ←                                                                                                      | ←   |     | ─   |      | ←    | ←    | -     | -     |       |
...
| 英字モード   | `X` + `C`       | レイヤー切替なしで OS 入力モードを英字に変更   | 全レイヤー   |
